# import multiprocessing, os, site, catboost
# # Force the "fork" start method so that subprocesses inherit the already loaded modules.
# # (The default "spawn" method starts a fresh Python interpreter and loses dynamic libraries like _catboost.)
# multiprocessing.set_start_method("fork", force=True)
# # Manually add CatBoost's shared library directory to LD_LIBRARY_PATH to ensure that subprocesses can locate the compiled _catboost library.
# site_packages = site.getsitepackages()[0]
# os.environ["LD_LIBRARY_PATH"] = f"{site_packages}/catboost/libs:" + os.environ.get("LD_LIBRARY_PATH", "")
# # Preload the CatBoost library in the main process, so child processes can inherit the loaded _catboost module.
# _ = catboost.CatBoostClassifier()
...
from lcdb import get_dataset_cate, get_entry_learner
    ...
    for (i, job) in enumerate(jobs_todo):
        ...
        if reload_data:
            ...
            if job['realistic'] == True: 
                X, y = get_dataset_cate(dataset, preprocess=False)
            else: 
                X, y = get_dataset_cate(dataset, preprocess=True)
        ...
        get_entry_pynisher = limit(get_entry_learner, wall_time=(timelimit, "h"), context='spawn')
        ...
        try:
            info = get_entry_pynisher(learner_name, learner_params, X, y, anchor, outer_seed, inner_seed, realistic, feature_scaling, encoder=encoder)
