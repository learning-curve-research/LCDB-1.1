            ...
            with open(str(file_path), 'r') as file:
                for line in file:
                    try:
                        ...
                        if status == 'ok':
                            results_array[idx_dataset, idx_learner, outer, inner, idx_anchor, fs_channel, raw_clean_channel] = 0
                        elif status == 'timeout':
                            results_array[idx_dataset, idx_learner, outer, inner, idx_anchor, fs_channel, raw_clean_channel] = 1
                        elif status == 'error':
                            results_array[idx_dataset, idx_learner, outer, inner, idx_anchor, fs_channel, raw_clean_channel] = 2  
                            error_array[idx_dataset, idx_learner, outer, inner, idx_anchor, fs_channel, raw_clean_channel] = json_obj.get('error_message', '')
...
error_array_obj = error_array.astype(object)

with h5py.File(output_filename, 'w') as f:

    f.create_dataset('status', data=results_array)

    dt = h5py.string_dtype(encoding='utf-8')  # HDF5 string
    f.create_dataset('error message', data=error_array_obj, dtype=dt)
